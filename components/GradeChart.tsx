...
import { GradeRecord, Exam, Student } from '../types.ts';
      ...
      <div className="h-80 w-full">
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
            ...
            <Tooltip 
              contentStyle={{ backgroundColor: 'rgba(255, 255, 255, 0.98)', borderRadius: '20px', border: 'none', boxShadow: '0 20px 25px -5px rgba(0, 0, 0, 0.1)', padding: '16px' }}
              labelStyle={{ fontSize: '14px', fontWeight: '900', color: '#1E293B', marginBottom: '10px' }}
            />
            <Legend verticalAlign="top" align="right" iconType="circle" wrapperStyle={{ fontSize: '11px', fontWeight: 'bold', paddingBottom: '30px' }} />
            {subjects.map((subj, idx) => (
              <Line key={subj} type="monotone" dataKey={subj} name={subj} stroke={colors[idx % colors.length]} strokeWidth={4} dot={{ r: 4, strokeWidth: 2, fill: '#fff', stroke: colors[idx % colors.length] }} />
            ))}
          </LineChart>
        </ResponsiveContainer>
      </div>
